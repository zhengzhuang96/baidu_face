...
public class BaiduFacePlugin implements FlutterPlugin, MethodCallHandler, ActivityAware {
  ...
  public static void registerWith(Registrar registrar) {
    final MethodChannel channel = new MethodChannel(registrar.messenger(), "baidu_face");
    channel.setMethodCallHandler(new BaiduFacePlugin());
  }
  ...
  public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
    ...
    switch (call.method) {
      case "init":
        ...
        FaceSDKManager.getInstance().initialize(activity, licenseID, "idl-license.face-android",null);
        ...
        if (requestCode == LIVENESS_REQ_CODE) {
          if (livenessResult != null) {
            ...
            if (resultCode == Activity.RESULT_OK) {
              System.out.println("mBitmap "+data.getStringExtra("bestImage"));
